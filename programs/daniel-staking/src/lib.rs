...
#[cfg(feature = "test-id")] //anchor build --features local-testing
...
pub mod daniel_staking {
    ...
    pub fn withdraw_nested(ctx: Context<WithdrawNested>) -> Result<()> {
        ...
        let cpi_ctx = CpiContext::new_with_signer(
            ctx.accounts.token_program.to_account_info(),      //program: which program to call
            token::Transfer {
                from: ctx.accounts.token_vault_nested_ata.to_account_info(),  //the nested ata holding tokens you want to withdraw
                to: ctx.accounts.token_vault.to_account_info(),               //the vault to receive the tokens
                authority: ctx.accounts.token_vault.to_account_info(),        //the vault is the authority of the nested ata
            },
            signer,
        );
        ...
        token::close_account(cpi_ctx)?;   //token_vault_nested_ata no longer valid after this call
    ...
    pub fn reclaim_mint_authority(ctx: Context<ReclaimMintAuthority>, nonce: u8) -> Result<()> {
        ...
        let cpi_ctx = CpiContext::new_with_signer(
            ctx.accounts.token_program.to_account_info(),
            token::SetAuthority {
                current_authority: ctx.accounts.token_vault.to_account_info(), //PDA of the vault is the current mint authority of x token
                account_or_mint: ctx.accounts.x_token_mint.to_account_info(),  //update the mint authority of x_token_mint
            },
            &signer,
        );
        token::set_authority(
            cpi_ctx,                                               //CPI context
            AuthorityType::MintTokens,                             //Authority type is mint authority, not freeze or account close
            Some(ctx.accounts.token_mint.mint_authority.unwrap()), //new authority is the authority of the step token mint
        )?;
        Ok(())
    }

    pub fn stake(ctx: Context<Stake>, nonce: u8, amount: u64) -> Result<()> {
        let total_token = ctx.accounts.token_vault.amount; //how many STEP tokens total in vault
        let total_x_token = ctx.accounts.x_token_mint.supply; //how many xSTEP tokens total exist
        ...
        if total_token == 0 || total_x_token == 0 {
            //no math read, we mint them the amount they sent us
            let cpi_ctx = CpiContext::new_with_signer(
                ctx.accounts.token_program.to_account_info(),
                token::MintTo {
                    mint: ctx.accounts.x_token_mint.to_account_info(), //the x token mint
                    to: ctx.accounts.x_token_to.to_account_info(),     //the user's x token account
                    authority: ctx.accounts.token_vault.to_account_info(), //PDA vault is the mint authority
                },
                &signer,
            );
    ...
    pub fn unstake(ctx: Context<Unstake>, nonce: u8, amount: u64) -> Result<()> {
        ...
        emit!(PriceChange {         //client can listen for this event to track price changes
            old_step_per_xstep_e9: old_price.0,
            old_step_per_xstep: old_price.1,
            new_step_per_xstep_e9: new_price.0,
            new_step_per_xstep: new_price.1,
